...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput(inputId = "WRV", label = "Choose a Water Retention Value", value = 1.71, min = 1.5, max = 1.9),
      br(),
      textOutput("TimeDBMC"),
      textOutput("TimeWRV"),
      br(),
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("Description",
                           h3("Collected Data"),
                           "The collected data comes from my own experiments on paper drying using ultrasound mechanism. The data set includes the total time of drying for different inital dry-basis moisture contents (DBMC) of the paper and different water retention values (WRV). The dry-basis moisture content is described by the ratio of the weight of water to the weight of the dry sample and water retention value is a measure that determines the ability of the paper to retain the water. It is important to be able to predict the relationship between the total time of drying and the initial DBMC and WRV and it is also very expensive to conduct the experiments for all the data points. Therefore, I have decided to use single variable and multiple variable linear regression methods to predict the behavior between the parameters. The data set includes the results of 23 experiments and there are three levels for initial DBMC and also water retention value. Thpe trend is very obvious and the user is left to decide if the predicted total time of drying is reasonable for their product or not. ",
                           br(),
                           br(),
                           "The current technology for paper drying is using hot air impinging jet nozzles, which is not energy efficient and it consumes 70% of the total energy in the industry. Therefore, it is very important and necessary to develop new technologies that are more energy efficient and environment friendly. As a mechanical engineer who works in the field of heat and mass transfer, I am focused on developing new drying technologies. One part of my research is considering a novel drying method using ultrsound mechanism. In general, this technology applies the ultrasound vibrations to the wet sample and due to the atomization, the moisture can be removed from the paper. There are different effective parameters in the drying phenomena that need to be considered. Two of the most important parameters are the initial moisture content of the sample and the water retention value. My research has been proved that ultrasound mechanism is more effective in higher moisture content levels. In addition, higher water retention values mean the ability of the fiber to retain the water is higher and therefore the drying time is higher. The outcome of this research is very important for paper industry.",
                           br(),
                           br(),
...
have a significant impact on the energy consumption at the national level. Hence, in my research, I am developing a novel drying technology to reduce the energy consumption, globally. The results of this linear regression analysis would help industry to redict the total time of ultrasond drying at a given initial DBMC and WRV, in the range of the studied parameters.",
                           br(),
                           br(),
                           h3("Data Analysis"),
                           "R programming is used for the data analysis using simple built-in linear regression function. The libraries 'ggplot2' and 'plotly' are used for 2D and 3D visualization, respectively. The libraries 'caret' and 'plsdepot' are used for splitting the data into training and test sets to observe the effect of data on the regressions. The 'shiny' library is also utilized to build the interactive web interface so a user can interact with the data. To do the analysis, the user can simply click select new training data. The training data is specified to be 75% of the full data set. The user can click the button again at any time to randomly select new data points and observe the change in predicions. Two slider bars are present, allowing the user to pick values of temperature and pressure and evaluate the predicted total time of drying. The 'lr' command is used to perform the linear regression on the training data. The 'lr' command returns coefficient for the linear regression equation, which are then used to plot the results.",
                           br(),
                           br(),
                           "Three plots are presented. The second tab shows the plots for the single variable linear regression, one for initial DBMC only and one for WRV only. These results assume that there is only one dependent variable on the total time of drying. The sliders will adjust the predicted point on the trend line to show the total time of drying. A similar plot is done in 3D for observing the effect of both initial DBMC and WRV on total time of drying. In the 3D plot shown in the left pane, the trend is plotted as a surface, and the red prediction point moves along the surface based on the slider input. The graphic can also be use interactively to zoom in and rotate the view. The legend on the 3D plot also applies to the 2D plots. The numeric result of predicted total time of drying for each regression is also displayed in the left pane.",
                           br(),
                           br(),
                           "The third tab, model evaluation, includes one plot that compares the predicted value for total time of ultrasound drying with the reference value of total time of drying. If the slope of the plot is closer to 1, it means that the model prediction aligns well with the reference value. The linear regression is used to find an equation for descibing the dependent variable (total time of drying) to the independent variables (intitla DBMC and water retention value). This equation is a linear combination of coefficients and the independent variables. For the case of one single dependent variable and one single independent variable, the equations looks as follows:",
                           #insert regression equation here
                           "$$y = a_{0} + a_{1}x$$",
                           "where $y$ is the dependent variable, $a_{0}$ is the intercept, $x$ is the independent variable, and $a_{1}$ is the coefficient of the independent variable the is descibing how $y$ changes with $x$. This equation can be extended to n independent variables.",
                  ...
                  tabPanel("Model Evaluation",
                           withMathJax(),
                           br(),
                           "It seems that the results of multiple linear regression analysis is more accurate for the colected data set compare to the single variable linear regression analysis. Therefore, the model evaluation is performed on multiple variable linear regression both for initial DBMC and WRV. The plot below represents the the predicted total time of drying from the linear regression vs the actual total time of drying. Since the linear model predicts continuously and the total time of drying is integer only, the values will almost never coincide perfectly.",
                           br(),
                           br(),
                           plotOutput("test"),
                           br(),
...
shiny::runGitHub("ZahraNoori", "ZahraNoori2021",subdir = "ZahraNoori",ref="main")
#shiny::runApp("ZahraNoori2021")
